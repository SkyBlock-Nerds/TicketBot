package net.hypixel.nerdbot.tickets.config.tooling;
...
public class ConfigGenerator {
    ...
    public static void main(String[] args) {
        ...
        try {
            TicketBotConfig config = generator.generate(TicketBotConfig.class);

            // Exclude fields inherited from DiscordBotConfig so only TicketBot-specific fields are serialized
            Gson gson = DataSerialization.GSON.newBuilder()
                .addSerializationExclusionStrategy(new ExclusionStrategy() {
                    @Override
                    public boolean shouldSkipField(FieldAttributes f) {
                        return f.getDeclaringClass() == DiscordBotConfig.class;
                    }
    ...
    private List<Field> getAllFields(Class<?> clazz) {
        ...
        // Only include TicketBot-specific fields
        while (current != null && current != Object.class && current != DiscordBotConfig.class) {
